...
const coordsWireStringArray1 = coordinatesWire1.map(coordinates => coordinates.toString());
const coordsWireStringArray2 = coordinatesWire2.map(coordinates => coordinates.toString());
let allIntersections = [];
for (var i in coordinatesWire1) {
    if (coordsWireStringArray2.indexOf(coordsWireStringArray1[i]) !== -1) {
        allIntersections.push(coordsWireStringArray1[i]);
    }
}
...
console.log(getFewestCombinedSteps(coordsWireStringArray1, coordsWireStringArray2, allIntersections));
...
function getFewestCombinedSteps(coordinates1, coordinates2, intersections) {
    let fewestSteps;
    intersections.forEach(intersection => {
        const interSectionDistance = getCombinedStepsToCentralPort(coordinates1, coordinates2, intersection);
        if (fewestSteps <= interSectionDistance) {
            return;
        }
        fewestSteps = interSectionDistance;
    });
    return fewestSteps;
}

function getCombinedStepsToCentralPort(coords1, coords2, intersection) {
    const stepsWire1 = coords1.indexOf(intersection);
    const stepsWire2 = coords2.indexOf(intersection);
